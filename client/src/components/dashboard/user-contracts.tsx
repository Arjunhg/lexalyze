  ...
  const columns: ColumnDef<ContractAnalysis>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
            <Badge className="rounded-md" variant={score > 70 ? "default" : score < 50 ? "destructive" : "secondary"}>{score.toFixed(2)} Overall Score</Badge>
        );
      },
    },
    {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="bg-gradient-to-br from-white to-gray-50 shadow-lg hover:shadow-xl transition-all duration-300">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 border-b border-gray-100 pb-4">
                    ...
                    <Info className="h-5 w-5 text-blue-600 cursor-pointer" aria-label="Total number of contracts" />
                </CardHeader>
                <CardContent className="py-6">
                    <div className="text-4xl font-bold text-gray-800">{totalContracts}</div>
                </CardContent>
            </Card>
            ...
            <Card className="bg-gradient-to-br from-white to-gray-50 shadow-lg hover:shadow-xl transition-all duration-300">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 border-b border-gray-100 pb-4">
                    ...
                    <Info className="h-5 w-5 text-purple-600 cursor-pointer" aria-label="Average contract score" />
                </CardHeader>
                <CardContent className="py-6">
                    <div className="text-4xl font-bold text-gray-800">{averageScore.toFixed(2)}</div>
                </CardContent>
            </Card>

            <Card className="bg-gradient-to-br from-white to-gray-50 shadow-lg hover:shadow-xl transition-all duration-300">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 border-b border-gray-100 pb-4">
                    <CardTitle className="text-lg font-semibold bg-gradient-to-r from-red-600 to-orange-600 bg-clip-text text-transparent">
                        High Risk Contracts
                    </CardTitle>
                    <Info className="h-5 w-5 text-red-600 cursor-pointer" aria-label="Number of high risk contracts" />
                </CardHeader>
                <CardContent className="py-6">
                    <div className="text-4xl font-bold text-gray-800">{highRiskContracts}</div>
                </CardContent>
            </Card>
      </div>
